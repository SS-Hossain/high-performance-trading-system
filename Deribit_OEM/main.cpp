...
std::string static readAccessToken(const std::string& filename) {
    std::cout << "Attempting to open file: " << filename << std::endl;

    if (!std::filesystem::exists(filename)) {
        std::cerr << "File does not exist at the specified path!" << std::endl;
        return "";
    }
    ...
    std::getline(file, accessToken); // Read token if file opens successfully
    return accessToken;
}
...
int main()
{
    std::string accessToken = readAccessToken("D:\\Deribit_TestAPI_GoQuant\\GoQuantOEMSApp\\access_token.txt");

    std::ifstream file("D:\\Deribit_TestAPI_GoQuant\\GoQuantOEMSApp\\access_token.txt");
    if (!file) {
        std::cerr << "File cannot be opened!" << std::endl;
    }
    else {
    ...
    if (!accessToken.empty()) {
        ...
        std::cout << "Failed to read access token." << std::endl;
    }
    std::cout << "Hello, GoQuantOEMSApp!" << std::endl;
